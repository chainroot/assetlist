...
const traverseDirectories = async (directory, replace) => {
...
const retryDownloads = async () => {
    ...
    for (const oldUrl of retryUrls) {
        ...
        if (NON_COSMOS_CHAINS.includes(chain)) {
            ...
            if (await downloadFile(nonCosmosUrl, path.join(RAW_IMAGE_PATH, `${hash}${extension}`)))
                continue;
        ...
        if (await downloadFile(cosmosUrl, path.join(RAW_IMAGE_PATH, `${hash}${extension}`)))
            continue;
