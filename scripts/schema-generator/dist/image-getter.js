...
const processJsonFiles = async (files, directory) => {
...
const traverseDirectories = (directory, replace) => {
...
const retryDownloads = async () => {
    ...
    for (const oldUrl of retryUrls) {
        ...
        if (NON_COSMOS_CHAINS.includes(chain)) {
            ...
            if (await downloadFile(nonCosmosUrl, path.join(RAW_IMAGE_PATH, asset)))
                continue;
            if (await downloadFile(nonCosmosBackupUrl, path.join(RAW_IMAGE_PATH, asset.endsWith('.png') ? asset.replace(/\.png$/, '.svg') : asset.replace(/\.svg$/, '.png'))))
                continue;
        }
        ...
        if (await downloadFile(cosmosUrl, path.join(RAW_IMAGE_PATH, asset)))
            continue;
        if (await downloadFile(cosmosBackupUrl, path.join(RAW_IMAGE_PATH, asset.endsWith('.png') ? asset.replace(/\.png$/, '.svg') : asset.replace(/\.svg$/, '.png'))))
            continue;
...
const replaceUrlsInJson = async (files, directory) => {
    for (const file of files) {
        const filePath = path.join(directory, file);
